...
export async function addGame(request: Request, env: AppEnv): Promise<Response> {
  ...
  try {
    await createGame(env.DB, {
      ...
      opponent_abbreviation: optionalText(form, "opponent_abbreviation", 10),
      venue: optionalText(form, "venue", 120),
      site: oneOf(form, "site", ["home", "away", "neutral"] as const),
...
export async function editGame(
  ...
  const changed = await updateGame(
    ...
    optionalText(form, "opponent_abbreviation", 10),
    optionalText(form, "venue", 120),
    oneOf(form, "site", ["home", "away", "neutral"] as const),
    utcTimestamp(form),
