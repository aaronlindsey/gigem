...
async function verifyAccessToken(token: string, env: AppEnv): Promise<string> {
...
export async function authenticatedEmail(request: Request, env: AppEnv): Promise<string> {
  if (env.ENVIRONMENT !== "production") {
    const localEmail = request.headers.get("x-local-auth-email") ?? env.LOCAL_AUTH_EMAIL;
    if (localEmail) return normalizeEmail(localEmail);
  }
